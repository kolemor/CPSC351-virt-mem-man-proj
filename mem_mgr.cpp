...
page_node tlb[TLB_SIZE]; // change this to list like lru_list
...
void lru_update(size_t page) {
    if (!lru_list.empty()) {
        auto itr = lru_list.begin();
        while (itr != lru_list.end()) {
            if (itr->npage == pg_table[page].npage) {
                lru_list.erase(itr++);
            } else {
                itr++;
            }
...
void lru_replace_page(size_t& frame) { 
    size_t page = lru_list.begin()->npage;
    frame = pg_table[page].frame_num;
    pg_table[page].is_used = false;
